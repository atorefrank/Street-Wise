...
from fetch_data import getImageData, error_rate, init_weight_and_bias, y2indicator
...
class  ConvPoolLayer(object):
	...
	def __init__(self, mi, mo, fw=5, fh=5, poolsz=(2, 2)):
		sz = (fw, fh, mi, mo)			# mi = input feature map size, mo = output feature map size
...
class CNN(object):
	...
	def  fit(self, X, Y, lr=10e-4, mu=0.99, reg=10e-4, decay=0.99999, eps=10e-3, batch_sz=30, epochs=3, show_fig=True):
		...
		#initializing convolution pool layers
		N, d, d, c = X.shape
		mi = c
		outw = d
		outh = d
		...
		with tf.Session() as session:
			...
			for i in xrange(epochs):
				...
				for j in xrange(n_batches):
					...
					Ybatch = Y[j*batch_sz:(batch_sz+batch_sz)]
